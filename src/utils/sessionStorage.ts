...
export function getFirstMeaningfulUserMessageTextContent<T extends Message>(
  ...
  for (const rawMsg of transcript) {
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (isTranscriptMessage(entry)) {
        const transcriptEntry = entry as any
        if (transcriptEntry.parentUuid && progressBridge.has(transcriptEntry.parentUuid)) {
          transcriptEntry.parentUuid = progressBridge.get(transcriptEntry.parentUuid) ?? null
        }
        messages.set(transcriptEntry.uuid, entry)
        ...
        const sideEntry = entry as any
        if (sideEntry.type === 'summary' && sideEntry.leafUuid) {
        summaries.set(sideEntry.leafUuid, sideEntry.summary)
      } else if (sideEntry.type === 'custom-title' && sideEntry.sessionId) {
        customTitles.set(sideEntry.sessionId, sideEntry.customTitle)
      } else if (sideEntry.type === 'tag' && sideEntry.sessionId) {
        tags.set(sideEntry.sessionId, sideEntry.tag)
      } else if (sideEntry.type === 'agent-name' && sideEntry.sessionId) {
        agentNames.set(sideEntry.sessionId, sideEntry.agentName)
      } else if (sideEntry.type === 'agent-color' && sideEntry.sessionId) {
        agentColors.set(sideEntry.sessionId, sideEntry.agentColor)
      } else if (sideEntry.type === 'agent-setting' && sideEntry.sessionId) {
        agentSettings.set(sideEntry.sessionId, sideEntry.agentSetting)
      } else if (sideEntry.type === 'mode' && sideEntry.sessionId) {
        modes.set(sideEntry.sessionId, sideEntry.mode)
      } else if (sideEntry.type === 'worktree-state' && sideEntry.sessionId) {
        worktreeStates.set(sideEntry.sessionId, sideEntry.worktreeSession)
      } else if (sideEntry.type === 'pr-link' && sideEntry.sessionId) {
        prNumbers.set(sideEntry.sessionId, sideEntry.prNumber)
        prUrls.set(sideEntry.sessionId, sideEntry.prUrl)
        prRepositories.set(sideEntry.sessionId, sideEntry.prRepository)
      } else if (sideEntry.type === 'file-history-snapshot') {
        fileHistorySnapshots.set(sideEntry.messageId, sideEntry)
      } else if (sideEntry.type === 'attribution-snapshot') {
        attributionSnapshots.set(sideEntry.messageId, sideEntry)
      } else if (sideEntry.type === 'content-replacement') {
        // Subagent decisions key by agentId (sidechain resume); main-thread
        // decisions key by sessionId (/resume).
        if (sideEntry.agentId) {
          const existing = agentContentReplacements.get(sideEntry.agentId) ?? []
          agentContentReplacements.set(sideEntry.agentId, existing)
          existing.push(...sideEntry.replacements)
        } else {
          const existing = contentReplacements.get(sideEntry.sessionId) ?? []
          contentReplacements.set(sideEntry.sessionId, existing)
          existing.push(...sideEntry.replacements)
        }
      } else if (sideEntry.type === 'marble-origami-commit') {
        contextCollapseCommits.push(sideEntry)
        } else if (sideEntry.type === 'marble-origami-snapshot') {
          contextCollapseSnapshot = sideEntry
        }
      }
    }
  } catch {

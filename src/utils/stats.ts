    ...
    for (const { sessionFile, entries, error, skipped } of results) {
      ...
      for (const entry of entries) {
        if (isTranscriptMessage(entry)) {
          messages.push(entry)
        } else if ((entry as any).type === 'speculation-accept') {
          totalSpeculationTimeSavedMs += (entry as any).timeSavedMs
        }
      }
...
/**
 * Aggregates stats from all Claude-code-laohuang sessions across all projects.

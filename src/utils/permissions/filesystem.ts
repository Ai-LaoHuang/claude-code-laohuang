...
// Always ask when Claude-code-laohuang tries to edit its own config files
 ...
 * This is a per-user temporary directory used by Claude-code-laohuang for all temp files.
 *
 ...
 * - MCP CLI state files (managed internally by Claude-code-laohuang)
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (!safetyCheck.safe) {
    ...
    const unsafe = safetyCheck as Extract<typeof safetyCheck, { safe: false }>
    return {
      behavior: 'ask',

...
export class ConversationService {
  ...
  private buildSessionCliArgs(
    ...
    return this.resolveCliArgs([
      ...
      ...worktreeArgs,
      ...(process.env.CC_HAHA_DESKTOP_STRICT_MCP_CONFIG === '1'
        ? ['--strict-mcp-config']
        : []),
  ...
  private resolveCliArgs(baseArgs: string[]): string[] {
    ...
    if (!launcher) {
      ...
      return [path.resolve(import.meta.dir, '../../../bin/claude-code-laohuang'), ...baseArgs]
    }
  ...
  private buildStartupError(
    ...
    if (
      ...
      return new ConversationStartupError(
        'Desktop chat could not start because Claude CLI is not authenticated. Run `./bin/claude-code-laohuang /login` or provide valid API credentials, then retry.',
        'CLI_AUTH_REQUIRED',
      )
    }

...
export function ManageMarketplaces({
  ...
  const getPendingCounts = () => {
    const updateCount = count(marketplaceStates, s => (s as MarketplaceState).pendingUpdate);
    const removeCount = count(marketplaceStates, s => (s as MarketplaceState).pendingRemove);
    return {
      updateCount,
      removeCount
    };
  ...
  if (internalView === 'details' && selectedMarketplace) {
    ...
    return <Box flexDirection="column">
        ...
        {!isUpdating && !shouldSkipPluginAutoupdate() && selectedMarketplace.autoUpdate && <Box marginTop={1}>
              <Text dimColor>
                Auto-update enabled. Claude-code-laohuang will automatically update this

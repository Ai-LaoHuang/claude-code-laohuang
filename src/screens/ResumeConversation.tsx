  ...
  async function onSelect(log_0: LogOption) {
    ...
    if (crossProjectCheck.isCrossProject) {
      if (!crossProjectCheck.isSameRepoWorktree) {
        const crossProjectCommand = crossProjectCheck as Extract<typeof crossProjectCheck, { command: string }>;
        const raw = await setClipboard(crossProjectCommand.command);
        if (raw) process.stdout.write(raw);
        setCrossProjectCommand(crossProjectCommand.command);
        return;
      }
    }
    try {

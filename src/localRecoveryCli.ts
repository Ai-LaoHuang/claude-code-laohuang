...
function printHelp(): void {
  process.stdout.write(
    [
      'Usage: claude-code-laohuang [options] [prompt]',
      '',
...
function printVersion(): void {
  process.stdout.write('0.1.0 (Claude-code-laohuang local recovery)\n')
}
...
async function runInteractive(parsed: {
  ...
  process.stdout.write(
    `Claude-code-laohuang local interactive mode\nmodel: ${model}\ncommands: /exit, /clear\n\n`,
  )
  rl.prompt()
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      const text = response.content
        ...
        .map(block => block.text)
        .join('\n')
      process.stdout.write(`laohuang> ${text}\n\n`)

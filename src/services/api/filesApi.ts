 ...
 * Used by the Claude-code-laohuang agent to download file attachments at session startup.
 *
...
async function retryWithBackoff<T>(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    const retryError = result as Extract<RetryResult<T>, { done: false }>
    lastError = retryError.error || `${operation} failed`
    logDebug(

...
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control needs a bridge auth token. Sign in with `/login` using a claude.ai account, or in this local repair build set `CLAUDE_BRIDGE_OAUTH_TOKEN` (and optionally `CLAUDE_BRIDGE_BASE_URL`).'

/** Full error printed when `claude remote-control` is run without auth. */
export const BRIDGE_LOGIN_ERROR =
  'Error: Remote Control is missing bridge authentication.\n\n' +

...
function isLocalBridgeRepairEnabled(): boolean {
  return isEnvTruthy(process.env.CLAUDE_CODE_LOCAL_ENABLE_BRIDGE)
}
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of Claude-code-laohuang (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`claude update\` to update.`
    }
  }
  return null
}

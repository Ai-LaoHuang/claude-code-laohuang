...
function AssistantMessageBlock(t0) {
  ...
  if (feature("CONNECTOR_TEXT")) {
    if (isConnectorTextBlock(param)) {
      const connectorParam = param as unknown as { connector_text: string }
      let t1;
      if ($[0] !== connectorParam.connector_text) {
        t1 = {
          type: "text",
          text: connectorParam.connector_text
        };
        $[0] = connectorParam.connector_text;
        $[1] = t1;
      } else {
        t1 = $[1];
      }

...
export function BackgroundTasksDialog({
  ...
  const runningBashCount = count(bashTasks, (_: any) => _.status === 'running');
  const runningAgentCount = count(remoteSessions, (__0: any) => __0.status === 'running' || __0.status === 'pending') + count(agentTasks, (__1: any) => __1.status === 'running');
  const runningTeammateCount = count(teammateTasks, (__2: any) => __2.status === 'running');
        ...
        {allSelectableItems.length === 0 ? <Text dimColor>No tasks currently running</Text> : <Box flexDirection="column">
            {teammateTasks.length > 0 && <Box flexDirection="column">
                {(bashTasks.length > 0 || remoteSessions.length > 0 || agentTasks.length > 0) && <Text dimColor>
                    ...
                    {count(teammateTasks, (i: any) => i.type !== 'leader')})
                  </Text>}

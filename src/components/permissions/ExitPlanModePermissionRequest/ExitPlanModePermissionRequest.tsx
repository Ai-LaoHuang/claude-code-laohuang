...
export function ExitPlanModePermissionRequest({
  ...
  const imageAttachments = Object.values(pastedContents).filter((c): c is any => (c as any).type === 'image');
...
export function buildPlanApprovalOptions({
  ...
  if (showUltraplan) {
    options.push({
      label: 'No, refine with Ultraplan on Claude-code-laohuang on the web',
      value: 'ultraplan'
    });
  }
  options.push({

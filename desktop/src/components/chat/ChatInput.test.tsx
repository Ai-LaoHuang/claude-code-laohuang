...
describe('ChatInput file mentions', () => {
  ...
  it('uses the persisted message count without showing the separate project chip while history loads', async () => {
    useSessionStore.setState({
      sessions: [{
        id: sessionId,
        title: 'Project',
    ...
    expect(await screen.findByTestId('chat-input-panel')).toBeInTheDocument()
  ...
  it('allows the default composer to send a new empty session after launch controls are hidden', async () => {
    useSessionStore.setState({
      sessions: [{
        id: sessionId,
        title: 'Project',
    ...
    const input = screen.getByRole('textbox') as HTMLTextAreaElement
    fireEvent.change(input, { target: { value: '现在可以回复信息吗', selectionStart: 10 } })

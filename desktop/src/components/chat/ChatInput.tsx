...
export function ChatInput({ variant = 'default', compact = false }: ChatInputProps) {
  ...
  const useCodexComposer = !isMobileComposer
  ...
  const canSubmit = !isWorkspaceMissing &&
    !launchTransitioning &&
    (!embedLaunchControlsInHero || launchReady || !!pendingSlashUiAction) &&
  ...
  const handleSubmit = async () => {
    ...
    if (embedLaunchControlsInHero && (!launchReady || launchTransitioning)) return
    ...
    if (embedLaunchControlsInHero && activeLaunchWorkDir && launchBranch) {
  ...
  return (
    ...
    >
      <div
        className={
          isHeroComposer
            ? 'mx-auto flex w-full max-w-[920px] flex-col'
          : compact
              ? 'mx-auto max-w-full'
      ...
      >
        <div
          ...
          className={isHeroComposer
            ? `${useCodexComposer ? 'codex-composer-panel' : 'glass-panel'} relative flex flex-col gap-0 overflow-visible ${embedLaunchControlsInHero ? 'rounded-xl' : 'rounded-t-xl rounded-b-none'} p-0 transition-colors ${isDragActive ? 'composer-drop-target-active' : ''}`
            : compact
              ? `${useCodexComposer ? 'codex-composer-panel' : 'glass-panel'} relative overflow-visible p-3 transition-colors ${isMobileComposer ? 'rounded-2xl shadow-[0_-12px_36px_rgba(54,35,28,0.12)]' : 'rounded-xl'} ${isDragActive ? 'composer-drop-target-active' : ''}`
              : `${useCodexComposer ? 'codex-composer-panel' : 'glass-panel'} relative overflow-visible transition-colors ${isMobileComposer ? 'rounded-2xl p-3 shadow-[0_-12px_36px_rgba(54,35,28,0.12)]' : 'rounded-xl p-4'} ${isDragActive ? 'composer-drop-target-active' : ''}`}
          {...dragHandlers}
        >
          {isDragActive && (
            <ComposerDropOverlay
          ...
          {isHeroComposer ? (
            <div className="flex items-start gap-3">
              <textarea
                ...
                rows={2}
                className={`${useCodexComposer ? 'codex-composer-textarea' : ''} flex-1 resize-none border-none bg-transparent py-2 leading-relaxed text-[var(--color-text-primary)] outline-none placeholder:text-[var(--color-text-tertiary)] disabled:opacity-50`}
              />
            </div>
          ) : (
            <textarea
              ...
              disabled={isWorkspaceMissing}
              rows={1}
              className={`${useCodexComposer ? 'codex-composer-textarea' : ''} w-full resize-none bg-transparent text-sm leading-relaxed text-[var(--color-text-primary)] outline-none placeholder:text-[var(--color-text-tertiary)] disabled:opacity-50 ${
          ...
          <div className={isHeroComposer
            ? `${useCodexComposer ? 'codex-composer-toolbar' : ''} flex items-center justify-between border-t border-[var(--color-border-separator)] pt-3`
            : `${useCodexComposer ? 'codex-composer-toolbar' : ''} absolute bottom-0 left-0 right-0 flex items-center justify-between border-t border-[var(--color-border-separator)] ${
            ...
            <div className="flex min-w-0 items-center gap-2">
              {!isMemberSession && (
                <>
                  <div ref={plusMenuRef} className="relative">
                    <button
                      ...
                      className={useCodexComposer
                        ? 'codex-composer-plus-button'
                        : `text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-surface-hover)] ${isMobileComposer ? 'inline-flex h-11 w-11 items-center justify-center rounded-xl' : 'rounded-[var(--radius-md)] p-1.5'}`}
                    >
                  ...
                  <PermissionModeSelector compact={useCompactControls} variant={useCodexComposer ? 'codexComposer' : 'default'} />
                </>
              )}
            </div>
            ...
            <div className="flex min-w-0 items-center gap-2">
              {!isMemberSession && activeTabId && (
                <ContextUsageIndicator
                  ...
                  variant={useCodexComposer ? 'codexComposer' : 'default'}
                />
              )}
              {!isMemberSession && activeTabId && (
                <ModelSelector runtimeKey={activeTabId} disabled={isActive} compact={useCompactControls} variant={useCodexComposer ? 'codexComposer' : 'default'} />
              )}
	              <button
                ...
                className={useCodexComposer
                  ? `codex-composer-run-button ${!isMemberSession && isActive ? 'codex-composer-run-button--stop' : ''}`
              ...
              >
                <span className={`material-symbols-outlined ${useCodexComposer ? 'text-[26px]' : 'text-[14px]'}`}>
                  {!isMemberSession && isActive ? 'stop' : useCodexComposer ? 'arrow_upward' : 'arrow_forward'}
                </span>
                {!useCodexComposer && !iconOnlyAction && (!isMemberSession && isActive ? t('common.stop') : isMemberSession ? t('common.send') : t('common.run'))}
              </button>
            </div>
          </div>

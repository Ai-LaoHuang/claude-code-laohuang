...
fn window_state_path(app: &AppHandle) -> Option<PathBuf> {
    ...
    // %APPDATA%\com.claude-code-laohuang.desktop\.
...
fn setup_system_tray(app: &mut tauri::App) -> tauri::Result<()> {
    let menu = MenuBuilder::new(app)
        .text(TRAY_SHOW_ID, "Show Claude Code LaoHuang")
        .separator()
        .text(TRAY_QUIT_ID, "Quit Claude Code LaoHuang")
        .build()?;

    let mut tray = TrayIconBuilder::with_id("main-tray")
        .tooltip("Claude Code LaoHuang")
        .menu(&menu)
...
fn resolve_app_root(_app: &AppHandle) -> Result<PathBuf, String> {
    ...
    // 的 src/desktop-server/index.ts 和 preload.ts，所以 Tauri 必须把整个 src/ +
...
fn adapter_config_path() -> Option<PathBuf> {
    if let Ok(config_dir) = std::env::var("CLAUDE_CONFIG_DIR") {
        let trimmed = config_dir.trim();
        if !trimmed.is_empty() {
            return Some(PathBuf::from(trimmed).join("adapters.json"));
        }
    }
    ...
    std::env::var("HOME")
        ...
        .map(|home| PathBuf::from(home).join(".claude").join("adapters.json"))
}

fn config_has_nonempty_string(config: &serde_json::Value, section: &str, key: &str) -> bool {
    config
        .get(section)
...
fn load_adapter_config_json() -> Option<serde_json::Value> {
    let path = adapter_config_path()?;
    let raw = fs::read_to_string(path).ok()?;
    serde_json::from_str(&raw).ok()
}

fn adapter_has_credentials(label: &str) -> bool {
    match label {
        "telegram" if nonempty_env("TELEGRAM_BOT_TOKEN") => return true,
        "feishu" if nonempty_env("FEISHU_APP_ID") && nonempty_env("FEISHU_APP_SECRET") => {
            return true
        }
        "wechat" if nonempty_env("WECHAT_ACCOUNT_ID") && nonempty_env("WECHAT_BOT_TOKEN") => {
            return true
        }
        "dingtalk"
            if nonempty_env("DINGTALK_CLIENT_ID") && nonempty_env("DINGTALK_CLIENT_SECRET") =>
        {
            return true
        }
        _ => {}
    ...
    let Some(config) = load_adapter_config_json() else {
        return false;
    };

    match label {
        "telegram" => config_has_nonempty_string(&config, "telegram", "botToken"),
        "feishu" => {
            config_has_nonempty_string(&config, "feishu", "appId")
                && config_has_nonempty_string(&config, "feishu", "appSecret")
        }
        "wechat" => {
            config_has_nonempty_string(&config, "wechat", "accountId")
                && config_has_nonempty_string(&config, "wechat", "botToken")
        }
        "dingtalk" => {
            config_has_nonempty_string(&config, "dingtalk", "clientId")
                && config_has_nonempty_string(&config, "dingtalk", "clientSecret")
        }
        _ => false,
    }
}
...
fn start_adapters_sidecars(app: &AppHandle) -> Result<Vec<CommandChild>, String> {
    ...
    for (label, flag) in [
        ...
        if !adapter_has_credentials(label) {
            println!("[claude-adapters:{label}] credentials not configured; skipping auto-start");
            continue;
        }
...
pub fn run() {
    ...
    let builder = builder
        .menu(|app| {
            let about_item =
                MenuItemBuilder::with_id("nav_about", "关于 Claude Code LaoHuang").build(app)?;
            let settings_item = MenuItemBuilder::with_id("nav_settings", "设置...")
                .accelerator("CmdOrCtrl+,")
                .build(app)?;

            let app_submenu = SubmenuBuilder::new(app, "Claude Code LaoHuang")
                .item(&about_item)
                .separator()
                .item(&settings_item)
                .separator()

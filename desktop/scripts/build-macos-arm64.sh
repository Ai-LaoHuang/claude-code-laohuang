...
APP_BUNDLE_NAME="Claude Code LaoHuang.app"
APP_BUNDLE_ID="com.claude-code-laohuang.desktop"

usage() {
  cat <<'EOF'
Build Claude Code LaoHuang desktop for macOS Apple Silicon and output a DMG.
...
tell application "Finder"
  tell disk "Claude Code LaoHuang"
    open
...
if [[ -n "${LATEST_APP}" ]]; then
  ...
  # Without this, opening target/.../bundle/macos/Claude Code LaoHuang.app directly
  ...
  CANONICAL_DMG="${CANONICAL_OUTPUT_DIR}/$(basename "${LATEST_DMG:-Claude Code LaoHuang_0.1.0_aarch64.dmg}")"
  build_canonical_dmg \

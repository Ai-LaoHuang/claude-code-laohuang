...
function Get-StagedArtifactName {
  ...
  switch -Regex ($ArtifactName) {
    ...
    '\.exe\.sig$' { return "Claude-Code-LaoHuang_${appVersion}_windows_x64_setup.exe.sig" }
    '\.exe$' { return "Claude-Code-LaoHuang_${appVersion}_windows_x64_setup.exe" }
    '\.msi\.zip\.sig$' { return "Claude-Code-LaoHuang_${appVersion}_windows_x64_msi.msi.zip.sig" }
    '\.msi\.zip$' { return "Claude-Code-LaoHuang_${appVersion}_windows_x64_msi.msi.zip" }
    '\.msi\.sig$' { return "Claude-Code-LaoHuang_${appVersion}_windows_x64_msi.msi.sig" }
    '\.msi$' { return "Claude-Code-LaoHuang_${appVersion}_windows_x64_msi.msi" }
...
$artifactPatterns = @('*.exe', '*.exe.sig', '*.msi', '*.msi.sig', '*.msi.zip', '*.msi.zip.sig', 'latest.json')
...
$buildInfo = @(
  ...
  "Windows installer (NSIS EXE): $nsisInstallerPath"

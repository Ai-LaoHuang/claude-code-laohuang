...
async function detectHostTriple() {
  ...
  try {
    proc = Bun.spawn(['rustc', '-vV'], {
      cwd: repoRoot,
      stdout: 'pipe',
      stderr: 'pipe',
    })
  } catch (error) {
    const fallbackTriple = detectHostTripleFromNode()
    if (fallbackTriple) {
      console.warn(
        `[build-sidecars] rustc unavailable; falling back to host triple ${fallbackTriple}`,
      )
...
function detectHostTripleFromNode() {
